      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <div className="flex items-center justify-between pb-4 mb-12 sm:mb-16 border-b border-[#D9DEE5] text-xs font-sans uppercase tracking-widest text-[#667085]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[#071A33] font-semibold">Conference Schedule</span>
          </div>
          <span>Two Full Days of Discourse</span>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          {/* Day Toggle Buttons */}
          <div className="inline-flex border border-[#D9DEE5] rounded-sm p-1 bg-[#F8F8F6] self-start md:self-auto">
            <button
              type="button"
              onClick={() => setActiveDay(1)}
              className={`px-5 py-2 text-xs font-sans uppercase tracking-wider font-semibold rounded-sm transition-all cursor-pointer ${
                activeDay === 1
                  ? 'bg-[#071A33] text-white shadow-subtle'
                  : 'text-[#667085] hover:text-[#071A33]'
              }`}
            >
              Day 01 — Inaugural &amp; Tracks 01-04
            </button>
            <button
              type="button"
        ...
        <div className="border border-[#D9DEE5] rounded-sm overflow-hidden bg-white shadow-subtle">
          <div className="divide-y divide-[#D9DEE5]">
            {currentSchedule.map((item, idx) => (
              <div
                key={idx}
                className="p-6 sm:p-7 hover:bg-[#F8F8F6] transition-colors flex flex-col lg:flex-row lg:items-start justify-between gap-6"
              >
                {/* Time & Type */}
                <div className="lg:w-1/4 shrink-0">
                  <div className="flex items-center gap-2 text-xs font-mono font-bold text-[#071A33] mb-1.5">
                    <Clock className="w-3.5 h-3.5 text-[#2563EB]" />
                    <span>{item.time}</span>
                  </div>
                  <span className="inline-block text-[11px] font-sans font-semibold uppercase tracking-wider px-2 py-0.5 rounded-xs bg-[#EFF6FF] text-[#2563EB] border border-[#BFDBFE]">
                    {item.type}
                  </span>
                </div>
                ...
                <div className="lg:w-2/4">
                  <h3 className="font-serif font-bold text-lg sm:text-xl text-[#071A33] mb-2 leading-snug">
                    {item.title}
                  </h3>
                ...
                <div className="lg:w-1/4 flex lg:justify-end items-center text-xs text-[#667085] font-sans font-medium">
                  <div className="flex items-center gap-1.5 bg-[#F8F8F6] border border-[#D9DEE5] px-3 py-1.5 rounded-sm">
